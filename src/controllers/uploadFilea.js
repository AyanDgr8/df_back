    ...
    try {
        ...
        if (proceed && newRecords.length > 0) {
            try {
                ...
                const loginResponse = await axios.post('http://localhost:8443/login', {
                    email: "ayandgr5@gmail.com",
                    password: "Ayan1012"
                }, {
                    headers: {
                ...
                if (!authToken) {
                    ...
                    for (const record of newRecords) {
                        ...
                        try {
                            await axios.post('http://localhost:8443/schedule-message', {
                                instance_id: "AyanDGR8",
                                recipient: formattedNumber,
                                message: "Welcome to Multycomm Interactive Media Private Limited !!"
                            }, {
                                headers: {
                                    'Content-Type': 'application/json',
